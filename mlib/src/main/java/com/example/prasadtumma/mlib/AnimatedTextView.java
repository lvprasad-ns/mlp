package com.example.prasadtumma.mlib;
...
/**
 * Created by prasad.tumma on 21-03-2016.
 */
public class AnimatedTextView extends TextView implements Runnable, ValueAnimator.AnimatorUpdateListener {
    ...
    public AnimatedTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context.getTheme().obtainStyledAttributes(attrs, R.styleable.AnimatedTextView, 0, 0));
    }
    ...
    protected void init(TypedArray attrs) {
        try {
            animationDuration = attrs.getInteger(R.styleable.AnimatedTextView_rtv_duration, animationDuration);
            text = attrs.getString(R.styleable.AnimatedTextView_android_text);
        } finally {
            attrs.recycle();
        }
